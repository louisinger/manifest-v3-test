import { confidential, Psbt, Transaction, networks, address, payments, crypto, AssetHash, script, bip341 } from 'liquidjs-lib';
export * from 'liquidjs-lib';
import { Psbt as Psbt$1 } from 'liquidjs-lib/src/psbt';
import { setDefaultWordlist, validateMnemonic, mnemonicToSeedSync, generateMnemonic } from 'bip39';
...
function getNetwork(str) {
  return str ? networks[str] : networks.liquid;
}
/**
    ...
    return runtime_1.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            ...
            return _context.abrupt("return", _extends({}, utxo, {
              unblindData: {
                ...
                value: confidential.confidentialValueToSatoshi(utxo.prevout.value).toString(10),
          ...
          case 2:
            _context.next = 4;
            return confidential.unblindOutputWithKey(utxo.prevout, Buffer.from(blindPrivKey, 'hex'));
...
function getScripts(addresses) {
  return new Set(addresses.map(function (a) {
    return address.toOutputScript(a.confidentialAddress).toString('hex');
  }));
}
...
var Identity = /*#__PURE__*/function () {
  function Identity(args) {
    ...
    this.network = networks[args.chain];
    this.type = args.type;
        ...
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              ...
              transaction = Transaction.fromHex(psetToUnsignedHex(psetBase64)); // set the outputs map
            ...
            case 24:
              ...
              witness = Transaction.fromBuffer(input.nonWitnessUtxo).outs[vout];
            ...
            case 35:
              ...
              _context.next = 38;
              return confidential.unblindOutputWithKey(input.witnessUtxo, privKey);
            ...
            case 43:
              _context.next = 45;
              return pset.blindOutputsByIndex(Psbt$1.ECCKeysGenerator(this.ecclib), inputsData, outputsKeys);
...
var MasterPublicKey = /*#__PURE__*/function (_Identity) {
  ...
  function MasterPublicKey(args) {
    ...
    checkIdentityType(args.type, IdentityType.MasterPublicKey); // validate xpub
    ...
    _this.masterBlindingKeyNode = SLIP77Factory(args.ecclib).fromMasterBlindingKey(args.opts.masterBlindingKey);
...
var Mnemonic = /*#__PURE__*/function (_MasterPublicKey) {
  ...
  function Mnemonic(args) {
    ...
    checkIdentityType(args.type, IdentityType.Mnemonic); // check set the language if it is different of the default language.
    // the "language exists check" is delegated to `bip39.setDefaultWordlist` function.

    setDefaultWordlist(args.opts.language || 'english');
    checkMnemonic(args.opts.mnemonic); // retreive the wallet's seed from mnemonic

    var walletSeed = mnemonicToSeedSync(args.opts.mnemonic, args.opts.passphrase); // generate the master private key from the wallet seed

    var network = networks[args.chain];
    ...
    var masterBlindingKeyNode = SLIP77Factory(args.ecclib).fromSeed(walletSeed);
  ...
  Mnemonic.Random = function Random(chain, ecclib, baseDerivationPath) {
    var randomMnemonic = generateMnemonic();
    return new Mnemonic({
      chain: chain,
      ecclib: ecclib,
...
var PrivateKey = /*#__PURE__*/function (_Identity) {
  ...
  function PrivateKey(args) {
    ...
    checkIdentityType(args.type, IdentityType.PrivateKey); // decode signing key pair from WIF
...
var BrowserInject = /*#__PURE__*/function (_Identity) {
  ...
  function BrowserInject(args) {
    ...
    checkIdentityType(args.type, IdentityType.Inject); //checks if we are in the brower and if the provider is injected in the dom
...
function p2msPayment(keys, blindingKey, required, network) {
  ...
  var multisigPayment = payments.p2wsh({
  ...
  var _blindingKey$derive = blindingKey.derive(address.toOutputScript(multisigPayment.address, network)),
...
function cosignerToXPub(cosigner, network, ecclib) {
  ...
  var walletSeed = mnemonicToSeedSync(cosigner.mnemonic, cosigner.passphrase);
...
var Multisig = /*#__PURE__*/function (_MultisigWatchOnly) {
  ...
  function Multisig(args) {
    ...
    var walletSeed = mnemonicToSeedSync(args.opts.signer.mnemonic, args.opts.signer.passphrase);
    var network = networks[args.chain];
        ...
        while (1) {
          switch (_context.prev = _context.next) {
            ...
            case 2:
              ...
              prevout = Transaction.fromHex(prevoutHex).outs[outpoint.vout];
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 3:
            ...
            addressPublicKey = address.fromConfidential(confidentialAddress).blindingKey;
          ...
          switch (_context16.prev = _context16.next) {
            ...
            case 2:
              ...
              makePrevout = function makePrevout(_ref6) {
                ...
                var prevout = Transaction.fromHex(hex.hex).outs[vout];
                return makeOutput$1({
                  txid: txid,
                  vout: vout
                }, prevout);
...
function craftSingleRecipientPset(unspents, recipient, coinSelector, changeAddressByAsset, substractFeeFromRecipient, satsPerByte) {
  ...
  var network = address.getNetwork(recipient.address);
...
function createFeeOutputFromPset(psetBase64, firstSelection, recipients, satsPerByte) {
  ...
  var feeAssetHash = address.getNetwork(recipients[0].address).assetHash;
...
function addToTx(psetBase64, unspents, recipients) {
  ...
  for (var _iterator = _createForOfIteratorHelperLoose(recipients), _step; !(_step = _iterator()).done;) {
    var _step$value = _step.value,
        ...
        address$1 = _step$value.address;
    var script = address$1 === '' ? Buffer.alloc(0) : address.toOutputScript(address$1);
    pset.addOutput({
...
function outputIndexFromAddress(tx, addressToFind) {
  ...
  var recipientScript = address.toOutputScript(addressToFind);
...
function compileFromEcc(ecc) {
  ...
  var compilers = withoutEccCompilers.set(ScriptType.ELTR, makeCompileELTR(bip341.BIP341Factory(ecc)));
  return function (ast) {
...
function makeCompileELTR(bip341lib) {
  return function (ast) {
    ...
    var tree = bip341.toHashTree(leaves, true); // this is a trick for the bip341 functions (accept only prefixed keys)
    ...
    return {
      witnesses: function witnesses(leafScript) {
        ...
        var path = bip341.findScriptPath(tree, bip341.tapLeafHash(leaf));
...
var parseASMScript = function parseASMScript(text) {
  ...
  var asmScript = script.fromASM(str);
  return [{
    type: TypeAST.HEX,
...
export { BLOCKSTREAM_ESPLORA_ENDPOINT, BrowserInject, BufferMap, DEFAULT_BASE_DERIVATION_PATH, DEFAULT_SATS_PER_BYTE, Electrs, ElectrsBatchServer, Identity, IdentityType, MasterPublicKey, Mnemonic, Multisig, MultisigWatchOnly, PrivateKey, Wallet, addToTx, analyse, analyzeTapscriptTree, balances, checkIdentityType, checkMnemonic, craftMultipleRecipientsPset, craftSingleRecipientPset, createFeeOutput, decodePset, esploraTxToTxInterface, estimateTxSize, fetchAllTxs, fetchAllUtxos, fetchAndUnblindTxs, fetchAndUnblindTxsGenerator, fetchAndUnblindUtxos, fetchAndUnblindUtxosGenerator, fetchTx, fetchTxHex, fetchUtxos, fromAssetHash, fromXpub, getAsset, getIndexFromAddress, getNetwork, getSats, getScripts, getUnblindURLFromTx, greedyCoinSelector, groupBy, inputBlindingDataMap, isConfidentialOutput, isUnblindedOutput, isValidAmount, isValidExtendedBlindKey, isValidXpub, makeEvaluateDescriptor, makeUnblindURL, masterPubKeyRestorerFromEsplora, masterPubKeyRestorerFromState, mnemonicRestorerFromChain, mnemonicRestorerFromEsplora, mnemonicRestorerFromState, multisigFromEsplora, multisigWatchOnlyFromEsplora, outputBlindingPubkeysMap, privateBlindKeyGetter, psetToUnsignedHex, psetToUnsignedTx, restorerFromEsplora, restorerFromState, toAssetHash, toNumber, toOutpoint, toXpub, tryToUnblindUtxo, txsFetchGenerator, unblindOutput, unblindTransaction, utxosFetchGenerator, utxosFromTransactions, validate, walletFromAddresses, walletFromCoins };
//# sourceMappingURL=ldk.esm.js.map
